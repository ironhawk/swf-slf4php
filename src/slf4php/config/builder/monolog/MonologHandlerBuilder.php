...
abstract class MonologHandlerBuilder implements Builder {
	 ...
	 * @return \swf\slf4php\config\builder\monolog\MonologHandlerBuilder
	 */
	 ...
	 * @return \swf\slf4php\config\builder\Appender
	 */
	 ...
	 * @return \swf\slf4php\config\builder\MonologHandlerBuilder
	 */
	public function initFromJson($jsonObj, $envVars) {
		...
		if (isset($jsonObj->formatter)) {
			...
			Preconditions::checkArgument($reflection->implementsInterface("\swf\slf4php\config\builder\Builder"), "'builderClass' {} doesn't implement \\swf\\logging\\config\\builder\\Builder interface in appender def: {}", $formatterJsonObj->builderClass, $formatterJsonObj);
